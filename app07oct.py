...
if data_file and rules_file:
    ...
    def expand_range(expr, df_cols):
        ...
        if "to" in expr:
            ...
            if base and base2 and base.group(1) == base2.group(1):
                prefix = base.group(1)
                return [f"{prefix}{i}" for i in range(int(base.group(2)), int(base2.group(2)) + 1) if f"{prefix}{i}" in df_cols]
                ...
                for op in ["<=", ">=", "!=", "<", ">", "="]:
                    if op in part:
                        ...
                        try:
                            ...
                            if op == "<=": sub_mask &= col_vals <= val_num
    ...
    for _, rule in rules_df.iterrows():
        ...
        # --- Step 1: Handle Skip logic first ---
        if "skip" in check_types:
            ...
            try:
                if "then" not in condition.lower():
                    raise ValueError("Invalid skip format — missing 'then'")
            ...
            if check_type == "range":
                try:
                    ...
                    for col in related_cols:
                        ...
                        effective_mask = rows_to_check & valid_mask
                        out_of_range = ~col_vals.between(min_val, max_val) & effective_mask
                        offenders = df.loc[out_of_range, id_col]
